...
import com.example.asd_ass5_shop.adapters.SugistionAdapter
import com.example.asd_ass5_shop.adapters.mmPagerAdapter
import com.example.asd_ass5_shop.classs.modle.productModle
import com.example.asd_ass5_shop.databinding.ActivityProductBinding
...
class ProductActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.productname.text = objectProduct.name_en
        binding.productdescribtion.text = objectProduct.description_en
        binding.oldPrice.paintFlags = Paint.STRIKE_THRU_TEXT_FLAG
        binding.oldPrice.text = "${objectProduct.price}$"

        binding.newPrice.text =
            "${Math.round((objectProduct.price - (objectProduct.price * (objectProduct.offer / 100))))}$"
        binding.ratingBar.rating = objectProduct.rate.toFloat()
