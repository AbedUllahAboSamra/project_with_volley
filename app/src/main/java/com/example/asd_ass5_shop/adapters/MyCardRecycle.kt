...
import com.example.asd_ass5_shop.databinding.BagShopItemDesignBinding
...
class MyCardRecycle(var context: Context, var array: ArrayList<productModle>) :
    ...
    override fun onBindViewHolder(holder: myViewHolder, position: Int) {
        ...
        var price =
            Math.round((array[position].price - (array[position].price * (array[position].offer / 100))))
        var quantety = 1
        ...
        Picasso.get().load(array[position].image).into(holder.binding.productImage)
        holder.binding.productname.text = array[position].name_en
